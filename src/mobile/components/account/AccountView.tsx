...
import { CheckCircle2, MapPin, ChevronRight, LogOut, Info, Shield, MessageSquare, Bell, Zap, Heart, Search } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useAppContext } from '../../context';
import { AIVenueCard } from '../ai/AIVenueCard';

export function AccountView() {
  const { selectedCity, theme, toggleTheme, walkingMode, setWalkingMode, smartNotifs, setSmartNotifs, favorites, venues } = useAppContext();
  const [activeSection, setActiveSection] = useState<'settings' | 'favorites'>('settings');

  const favoriteVenues = useMemo(() => {
    const favs = venues.filter(v => favorites.has(v.id));
    // Sort: quiet first (most useful default)
    return favs.sort((a, b) => a.pct - b.pct);
  }, [venues, favorites]);
    ...
    <div className="h-full overflow-y-auto no-scrollbar bg-[var(--k-bg)]">
      ...
      <div className="p-6 text-center border-b border-[var(--k-border-s)] relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none"
             style={{ background: 'var(--k-mesh-1), var(--k-mesh-2)' }} />
        <div className="relative">
          <div className="relative w-[90px] h-[90px] mx-auto mb-4">
            <div className="w-full h-full rounded-[28px] bg-gradient-to-br from-[#ff4d6a]/15 to-[#a855f7]/15
                            flex items-center justify-center text-4xl border border-white/[0.06]
                            ring-2 ring-[var(--k-accent)]/20"
                 style={{ boxShadow: '0 4px 20px rgba(255,77,106,0.1)' }}>
              🧑
            </div>
          ...
          <h2 className="font-syne font-extrabold text-[24px] text-[var(--k-text)] tracking-[-0.025em]">Alex Rivera</h2>
      ...
      <div className="grid grid-cols-3 gap-2.5 p-4">
        {[
          ...
          <div key={stat.label} className="p-3.5 rounded-[20px] glass-chip text-center"
      ...
      {/* Segment control — Settings / Favorites */}
      <div className="px-4 pb-3">
        <div className="flex items-center gap-2">
          <button
            onClick={() => setActiveSection('settings')}
            className={cn(
              'flex-1 py-2.5 rounded-full text-[12px] font-bold transition-all text-center',
              activeSection === 'settings'
                ? 'bg-white/8 text-[var(--k-text)] border border-white/[0.08]'
                : 'text-[var(--k-text-f)] hover:text-[var(--k-text-m)]'
            )}
          >
            <Search className="w-3 h-3 inline mr-1.5" />Settings
          </button>
          <button
            onClick={() => setActiveSection('favorites')}
            className={cn(
              'flex-1 py-2.5 rounded-full text-[12px] font-bold transition-all text-center relative',
              activeSection === 'favorites'
                ? 'bg-[#ff4d6a]/15 text-[#ff4d6a] border border-[#ff4d6a]/20'
                : 'text-[var(--k-text-f)] hover:text-[var(--k-text-m)]'
            )}
          >
            <Heart className={`w-3 h-3 inline mr-1.5 ${activeSection === 'favorites' ? 'fill-[#ff4d6a]' : ''}`} />
            Favorites
            {favorites.size > 0 && (
              <span className="ml-1.5 min-w-[18px] h-[18px] inline-flex items-center justify-center rounded-full
                               bg-[#ff4d6a]/20 text-[9px] font-bold tabular-nums">
                {favorites.size}
              </span>
            )}
          </button>
      ...
      {activeSection === 'settings' && (
        <div className="p-4 space-y-5">
          {/* Current City */}
          <div>
            <h3 className="type-overline text-[#ff4d6a] mb-2.5">Location</h3>
          ...
          {/* Appearance */}
          <div>
            <h3 className="type-overline text-[var(--k-text-m)] mb-2.5">Appearance</h3>
            <div className="p-4 rounded-2xl bg-[var(--k-surface-solid)] border border-[var(--k-border)] shadow-[var(--k-card-shadow)]">
          ...
          {/* Smart Features */}
          <div>
            <h3 className="type-overline text-[var(--k-text-m)] mb-2.5">Smart Features</h3>
            <div className="rounded-[20px] bg-[var(--k-surface-solid)] border border-[var(--k-border)] shadow-[var(--k-shadow-sm)]
          ...
          {/* Alert Preferences — content-visibility for off-screen perf */}
          <div className="cv-auto">
            <h3 className="type-overline text-[var(--k-text-m)] mb-2.5">Alert Preferences</h3>
            <div className="rounded-[20px] bg-[var(--k-surface-solid)] border border-[var(--k-border)] shadow-[var(--k-shadow-sm)]
                            overflow-hidden divide-y divide-[var(--k-border-s)]">
              {[
          ...
          {/* General — content-visibility for off-screen perf */}
          <div className="cv-auto">
            <h3 className="type-overline text-[var(--k-text-m)] mb-2.5">General</h3>
            <div className="rounded-[20px] bg-[var(--k-surface-solid)] border border-[var(--k-border)] shadow-[var(--k-shadow-sm)]
                            overflow-hidden divide-y divide-[var(--k-border-s)]">
              {[
          ...
          {favoriteVenues.length > 0 ? (
            <>
              <div className="flex items-center gap-2 mb-1">
                <Heart className="w-3.5 h-3.5 text-[#ff4d6a] fill-[#ff4d6a]" />
              ...
              {favoriteVenues.map((venue, i) => (
                <AIVenueCard key={venue.id} venue={venue} index={i} showHHDeal />
              ))}
            </>
          ) : (
            <div className="text-center py-16">
              <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-[#ff4d6a]/10 to-[#a855f7]/10
                              flex items-center justify-center border border-white/[0.06]">
                <Heart className="w-8 h-8 text-[#ff4d6a]/40" />
              </div>
              <p className="text-[15px] font-semibold text-[var(--k-text)]">No saved venues yet</p>
              <p className="text-[12px] text-[var(--k-text-m)] mt-1.5 max-w-[220px] mx-auto leading-relaxed">
                Tap the heart on any venue to save it here for quick access
              </p>
            </div>
          )}
        </div>
