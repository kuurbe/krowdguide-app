...
function buildRecommendations(venues: Venue[]): Recommendation[] {
  ...
  if (quietest) {
    recs.push({
      ...
      type: 'quiet-window',
      title: `${quietest.name} has lowest load`,
      subtitle: `${quietest.pct}% capacity — recommended now`,
      action: 'Route there',
      venue: quietest,
  ...
  if (busiest && busiest.pct > 70) {
    recs.push({
      id: `peak-${busiest.id}`,
      type: 'peak-alert',
      title: `${busiest.name} at peak load`,
      subtitle: `${busiest.pct}% capacity — consider an alternate`,
      action: 'Show alternates',
      venue: busiest,
  ...
  if (gem) {
    recs.push({
      id: `gem-${gem.id}`,
      type: 'hidden-gem',
      title: `${gem.name} — low crowd, high signal`,
      subtitle: `${gem.rating}★ rating, ${gem.pct}% capacity`,
      action: 'View details',
      venue: gem,
